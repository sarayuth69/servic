...
Public Class manage_equipment
            ...
            While read_stock.Read
                datagridview_Stock.Rows.Add(read_stock("Stock_ID"), read_stock("Stock_name"),
                                        read_stock("Stock_deteil"), read_stock("Stock_remark"),
                                         ...
                                         read_stock("Number_devices"), read_stock("Stock_unit"))
            End While
        End If
    ...
    Private Sub btn_insert_Stock_Click(sender As Object, e As EventArgs) Handles btn_insert_Stock.Click
        'If chackdata(txt_Stock_na  Text, txt_stock_ID.Text, "insert") = False Then
        ...
        Else
            ...
            With com
                .Connection = conn_db
                .CommandType = CommandType.Text
                .CommandText = "SELECT COUNT(*) FROM stock_pos WHERE  stock_pos.Stock_name = '" & txt_Stock_name.Text.Trim & "' AND stock_pos.Stock_price = '" & txt_Stock_price.Text.Trim & "'"
                check = com.ExecuteScalar()
                If check > 0 Then
                    MsgBox("มีข้อมูลในระบบแล้ว")
                Else
                    .CommandText = "INSERT INTO stock_pos VALUE ('0','" & txt_Stock_name.Text & "','" & txt_Stock_deteil.Text & "',
                                '" & txt_Stock_remark.Text & "','" & ComboBox_Stock_status.Text & "','" & txt_Stock_price.Text & "','0','" & ComboBox_unit.Text & "',NOW(),NOW())"
    ...
    Private Sub btn_edit_Stock_Click(sender As Object, e As EventArgs) Handles btn_edit_Stock.Click
        'If chackdata(txt_Stock_na  Text, txt_stock_ID.Text, "update") = False Then
        '    MsgBox("มีข้อมูลอยู่แล้ว")
        'Else
        Dim check As Integer
        ...
        With com
            ...
            Else
                .CommandText = "UPDATE `stock_pos` SET `Stock_name`='" & txt_Stock_name.Text & "', `Stock_deteil`='" & txt_Stock_deteil.Text & "', 
                                    `Stock_remark`='" & txt_Stock_remark.Text & "', `Stock_status`='" & ComboBox_Stock_status.Text & "', `Stock_price`='" & txt_Stock_price.Text & "',`Stock_unit`='" & ComboBox_unit.Text & "',
                                    `Stock_date_edit`=NOW()
    ...
    Private Sub datagridview_Stock_CellMouseUp(sender As Object, e As DataGridViewCellMouseEventArgs) Handles datagridview_Stock.CellMouseUp
        ...
        ComboBox_unit.Text = datagridview_Stock.Rows(e.RowIndex).Cells("Column8").Value
    End Sub

    Private Sub BTN_search_equipment_Click(sender As Object, e As EventArgs) Handles BTN_search_equipment.Click
        datagridview_Stock.Rows.Clear()
        ...
        com.CommandText = "SELECT * FROM stock_pos WHERE stock_pos.Stock_ID LIKE '%" & check_text(text_search.Text.Trim) & "%' OR stock_pos.Stock_name LIKE '%" & check_text(text_search.Text.Trim) & "%' "
        read_data = com.ExecuteReader
        If read_data.HasRows = True Then
            While read_data.Read
                datagridview_Stock.Rows.Add(read_data("Stock_ID"), read_data("Stock_name"),
                                        read_data("Stock_deteil"), read_data("Stock_remark"),
                                         read_data("Stock_status"), read_data("Stock_price"),
                                         read_data("Number_devices"), read_data("Stock_unit"))
            End While
        End If
    End Sub
